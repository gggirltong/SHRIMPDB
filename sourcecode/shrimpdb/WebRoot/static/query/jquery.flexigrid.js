...
(function($) {
    $.addFlex = function(t, p) {
        ...
        p = $.extend({
            ...
            rpOptions: [10, 20, 40, 100], //可选择设定的每页结果数
            title: false, //是否包含标题
                ...
                $.ajax({
                    ...
                    error: function(data) { 
                    	try { 
                    		...
                    		else { alert("获取数据发生异常;") } g.hideLoading(); } 
                    	catch (e) { } }
                });
            },
            doSearch: function() {
        ...
        $('thead tr:first th', g.hDiv).each
			(
			 	function() {
			 	   ...
			 	   $(thdiv).css({ "text-align": this.align, "width": this.width + 'px' });
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'>快速检索：<input type='text' size='50' name='q' class='qsbox' /> <select name='qtype'>" + sopt + "</select> <input type='button' name='sscontent' style='margin:0 8px 0 4px' value='搜索' /><input type='button' name='qclearbtn' value='清空' /></div>");
        ...
        if (p.title) {
            ...
            if (p.showTableToggleBtn) {
                $(g.mDiv).append('<div class="ptogtitle" title="显示/隐藏 表格"><span></span></div>');
